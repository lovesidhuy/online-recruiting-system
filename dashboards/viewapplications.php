...
$categoryInfo = $categoryObj->getCategoryById($jobInfo['category_id'])->fetch_assoc();
...
<head>
  ...
  <style>
    ...
    input, select, button {
      margin: 5px; padding: 8px; border: 1px solid #ccc; border-radius: 5px;
    }
    button {
      background-color: #5c67f2; color: white; cursor: pointer; border: none;
    }
    button:hover { background-color: #434ac1; }
...
<div style="text-align: center; margin-top: 20px;">
  <button onclick="window.location.href='hr_dashboard.php'">Back to Dashboard</button>
</div>
...
<table>
  ...
  <tbody>
    ...
    ?>
    <tr>
      <td class="applicant-id"><?= htmlspecialchars($app['applicant_id']) ?></td>
      ...
      <td><?= htmlspecialchars($ai_result) ?></td>
      <td class="category-name"><?= htmlspecialchars($categoryInfo['category_name'] ?? 'Unknown') ?></td>
    </tr>
    <?php endforeach; ?>
  </tbody>
</table>
